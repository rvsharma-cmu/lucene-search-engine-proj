...
public class QryParser {
  ...
  private static Qry createOperator (String operatorName) {
    ...
    if (operatorNameLowerCase.startsWith("#near/") || 
    		operatorNameLowerCase.startsWith("#window/")) {
    	
    	int index = operatorNameLowerCase.indexOf("/");
    	operatorDistance = Integer.parseInt(operatorNameLowerCase.substring(index+1));
    	operatorNameLowerCase = operatorNameLowerCase.substring(0, index);
    }
    ...
    switch (operatorNameLowerCase) {
      ...
      case "#near": 
    	  operator = new QryIopNear(operatorNameLowerCase, operatorDistance);
    	  break;
    	  
      case "#window":
    	  operator = new QryIopWindow(operatorNameLowerCase, operatorDistance);
    	  break; 
  ...
  /**
   *  Get the index of the right parenthesis that balances the
   ...
   *  '#NEAR/1 ()' after stop words are removed; and unnecessary nodes
  ...
  private static Qry parseString (String queryString)
	...
	double weight = 1.0; // default query weight ; 
    ...
    while (queryString.length() > 0) {
      ...
      if(queryTree instanceof QryWSop) {
    	  ...
    	  int indexOfWeight = queryString.indexOf(" "); 
    	  weight = Double.parseDouble(queryString.substring(0, indexOfWeight).trim());
    	  queryString = (queryString.substring(indexOfWeight + 1)).trim();
      }
